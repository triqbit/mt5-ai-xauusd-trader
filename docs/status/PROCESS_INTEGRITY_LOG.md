...
- **CI Fast Validation Lint Violation:** Ruff RUF005 lint violation detected in `scripts/doctor.py` (line 675: `[str(venv_python)] + sys.argv` concatenation vs unpacking `[str(venv_python), *sys.argv]`). Under Jules06 process watcher boundaries, this is documented for observation and review rather than modifying application code directly.
