...
- **CI Fast Validation Ruff Violation:** Ruff linting detected RUF005 (`Consider [str(venv_python), *sys.argv] instead of concatenation`) in `scripts/doctor.py:675`. As per Jules06 role boundaries, code fixes are not applied directly; this issue is flagged for review.
