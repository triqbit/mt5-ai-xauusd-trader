...
- **CI Blockage via Lint Debt:** The latest graft (PR #1111) has introduced 377+ lint errors and 169 formatting violations. Because CI runs on the entire repo for every PR, standard contributions are now globally blocked by pre-existing debt in the grafted code. This effectively mandates a "bypass-only" merge environment, further eroding quality controls.
