...
**CI Failure Analysis (Detected 2026-06-09):**
- **Formatting Blockage:** `ruff format --check` fails on 120 files across `src/`, `scripts/`, and `tests/`. This indicates that monolithic grafts are being performed without style enforcement, blocking all subsequent PRs from passing CI.
- **Dependency Conflict:** A critical conflict exists between `gymnasium==1.3.0` and `stable-baselines3==2.8.0` (which requires `gymnasium<1.3.0`). This was likely introduced in the PR #1498 graft, breaking the environment for all users.
- **Root Cause:** Total system swaps via grafts bypass local pre-commit hooks and CI checks on the source branches, injecting broken states directly into `main`.
