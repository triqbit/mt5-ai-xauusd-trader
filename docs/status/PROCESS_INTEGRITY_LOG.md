...
- **CI Fast Validation Lint Failure:** Ruff RUF005 lint violation detected in `scripts/doctor.py` (line 675) during CI Fast Validation (`subprocess.run([str(venv_python)] + sys.argv)`). Per role limits ("DO NOT fix code directly; you only observe and report"), this issue is reported here for Jules02 / human resolution.
