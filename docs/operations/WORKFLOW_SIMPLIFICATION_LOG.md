...
### 13. Friction: Pull Request Triage and Acceptance Verification
**Current state:** Jules05 manually reviews every PR to ensure it meets the 4-pillar acceptance criteria defined in `docs/features/`.
**Proposed automation:** `Merge gates`. Implement a GitHub Action that parses PR descriptions for a "Verified Acceptance" block. The gate must cross-reference the feature name with `docs/features/` and ensure all pillars are checked before allowing auto-merge.
**Implementation owner:** Jules05
**Risk level:** Low
**Estimated time saved:** 15 minutes per PR

### 14. Friction: Model Promotion to Production
**Current state:** Qualitative and manual decision to move a trained model from the research environment to the production `models/` directory.
**Proposed automation:** `Branch promotion logic`. Implement an automated "Model Promotion Pipeline" where models are promoted only if they pass a standardized 30-day Walk-Forward Optimization (WFO) test with Sharpe Ratio > 2.0 and Max Drawdown < 15% in a "Sandbox" environment.
**Implementation owner:** Jules04
**Risk level:** Medium
**Estimated time saved:** 120 minutes per model cycle
