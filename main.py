...
from src.core.health import run_health_gate
...
def main() -> int:
    ...
    if not run_health_gate(cfg, connector):
        log.critical("System health check failed. Aborting startup.")
        return 1

    balance = connector.get_account_balance()
    trade_logger = TradeLogger(db_url=cfg.database_url)
    monitor = Monitor(cfg)
