...
def main() -> int:
    ...
    try:
        ...
        metrics = trade_logger.read_performance_report()
        if metrics and metrics.get("total_trades", 0) > 0:
            table = Table(title="📊 Session Performance Summary", show_header=True, header_style="bold magenta")
            ...
            console.print("\n[yellow]No trades completed this session.[/yellow]")

    finally:
        connector.disconnect()
        console.print("[bold blue]MT5 Connector shut down. Farewell![/bold blue]")
    return 0
