...
def run_live(
    ...
    while True:
        try:
            ...
            if df.empty or not tick["bid"] or not tick["ask"]:
                log.warning("Incomplete market data - retrying in %d seconds", poll_interval)
                time.sleep(poll_interval)
                continue

            # 2. Build observation vector
            # This is a simplified observation for the scaffold
...
def main() -> int:
    ...
    # Set environment variables for Pydantic config
    ...
    # Must provide placeholders for required Pydantic fields if not in .env
    if not os.getenv("MT5_PASSWORD"):
        os.environ["MT5_PASSWORD"] = "placeholder"
    if not os.getenv("MT5_SERVER"):
        os.environ["MT5_SERVER"] = "placeholder"
