...
### Implementation Note (v1.3) - Performance Indexing
- **Performance Lookup Optimization**: Added a composite index `ix_trades_perf_lookup` on `(status, is_deleted, created_at)` to the `trades` table. This optimizes the frequently executed queries in `TradeLogger.read_performance_report()`.
