    ...
    for display_name, val in deps.items():
        ...
        try:
            ...
            try:
                ...
                if actual_version != "unknown" and parse_version(actual_version) < parse_version(min_version):
                        outdated.append(
                            f"{display_name} (found {actual_version}, need {min_version})"
                        )
...
def check_talib():
    try:
        ...
        return DiagnosticCheck(
            "TA-Lib Library", "FAILED", "Python wrapper not installed", "Run 'pip install TA-Lib'"
        )
    except Exception as e:
        return DiagnosticCheck(
            "TA-Lib Library",
