...
def generate_synthetic_data(db_url):
    ...
    with Session() as session:
        ...
        for i in range(20):
            ...
            dt = now.replace(hour=10, minute=random.randint(0, 59)) - timedelta(days=random.randint(1, 30))
            sig = ModelSignal(
                symbol="XAUUSD",
                direction=1,
        ...
        for i in range(50):
            dt = now.replace(hour=2, minute=random.randint(0, 59)) - timedelta(days=random.randint(1, 10))
            sig = ModelSignal(
                symbol="XAUUSD",
