...
def create_mock_explanation(direction=SignalDirection.BUY):
    explanation = SignalExplanation(
        ...
        model_attributions=[
            ModelAttribution(model_name="PPO", vote=direction, confidence=0.9, weight=0.6, is_dominant=True),
            ModelAttribution(model_name="LSTM", vote=direction, confidence=0.8, weight=0.4, is_dominant=False)
        ],
        feature_contributions=[
            FeatureContribution(cluster_name="Momentum", contribution_score=0.7 if direction == SignalDirection.BUY else -0.7, impact_level="High", summary="Strong momentum alignment"),
            FeatureContribution(cluster_name="Volatility", contribution_score=0.2, impact_level="Low", summary="Low volatility environment"),
            FeatureContribution(cluster_name="Trend", contribution_score=-0.3 if direction == SignalDirection.BUY else 0.3, impact_level="Medium", summary="Counter-trend pressure")
        ],
...
def verify_ux():
    ...
    print("="*80)
    macro_risk_blocked = RiskStatus(is_blocked=True, risk_multiplier=0.0, active_events=[], reason="Blocked by FOMC")
    perf_metrics_low = {
        "sharpe_ratio": 0.8,
        "profit_factor": 1.1,
        "recovery_factor": 1.5,
    ...
    packet_blocked = dss.assemble_packet("XAUUSD", explanation, regime, macro_risk_blocked, perf_metrics_low)
