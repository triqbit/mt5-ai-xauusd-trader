...
def main():
    ...
    orchestrator.add_section(RegimeSection(
        ...
        regimes=[
            RegimeSummary(label="Bullish Trend", frequency_pct=45.2, avg_duration_bars=120, profitability="High"),
            RegimeSummary(label="Ranging", frequency_pct=30.8, avg_duration_bars=45, profitability="Neutral"),
            RegimeSummary(label="Mean Reversion", frequency_pct=24.0, avg_duration_bars=15, profitability="Low"),
        ],
    ...
    orchestrator.add_section(StressTestSection(
        ...
        baseline=StressedMetric(name="Baseline", total_return="18.2%", max_drawdown="4.5%", sharpe="2.4", recovery_factor="4.0", profit_factor="2.8", outcome="PASS"),
        scenarios=[
            StressedMetric(name="Execution Hell", total_return="12.1%", max_drawdown="6.8%", sharpe="1.9", recovery_factor="1.8", profit_factor="1.9", outcome="PASS"),
            StressedMetric(name="Liquidity Crisis", total_return="5.4%", max_drawdown="12.2%", sharpe="0.8", recovery_factor="0.4", profit_factor="1.2", outcome="PASS"),
            StressedMetric(name="Flash Crash", total_return="-8.5%", max_drawdown="22.4%", sharpe="-0.4", recovery_factor="0.0", profit_factor="0.7", outcome="FAIL"),
        ],
        fragility_indicators=["High sensitivity to wide spreads (>5 pips)", "Alpha decay during 5s+ latency spikes"],
    ...
    orchestrator.add_section(HyperparameterSection(
        ...
        parameters=[
            ParameterRobustness(name="Fast Window", range="5-20", optimal="12", sensitivity="Low"),
            ParameterRobustness(name="Slow Window", range="21-60", optimal="48", sensitivity="Medium"),
            ParameterRobustness(name="Confidence Threshold", range="0.5-0.9", optimal="0.75", sensitivity="High"),
        ],
    ...
    orchestrator.add_section(TradePatternSection(
        ...
        concentrations=[
            PatternConcentration(attribute="Session", value="London", win_rate=0.62, profit_factor=2.4, total_trades=145),
            PatternConcentration(attribute="Algorithm", value="Ensemble_V2", win_rate=0.58, profit_factor=2.1, total_trades=210),
        ],
        behavioral_risks=[
            BehavioralRisk(type="Overtrading", description="Sydney session trade frequency 2x baseline with negative edge."),
            BehavioralRisk(type="Toxic Motif", description="PPO_Agent Buy in Extreme Volatility shows 15% win rate.")
        ],
        motifs=[
            SignalMotif(algorithm="PPO", direction=1, volatility_bucket="Extreme", confidence_bucket="Low", session="Sydney", frequency=12, win_rate=0.15)
        ],
    ...
    orchestrator.add_section(ModelDriftSection(
        metrics=[
            DriftMetric(name="Feature: RSI_14", baseline="0.52", current="0.58", drift_pct=11.5, status="STABLE"),
            DriftMetric(name="Feature: ATR_Ratio", baseline="1.2", current="2.1", drift_pct=75.0, status="CRITICAL"),
        ],
    ...
    orchestrator.add_section(BenchmarkSection(
        comparisons=[
            BenchmarkComparison(name="Buy & Hold", total_return="8.5%", sharpe="0.9", max_drawdown="15.2%", p_value="0.001"),
            BenchmarkComparison(name="EMA Crossover", total_return="12.4%", sharpe="1.4", max_drawdown="8.5%", p_value="0.015"),
        ],
    ...
    orchestrator.add_section(RareEventSection(
        scenarios=[
            RareEventSummary(event_type="Flash Crash", peak_impact_pct=-0.082, realized_volatility=0.045, recovery_attained=0.65),
            RareEventSummary(event_type="Liquidity Vacuum", peak_impact_pct=-0.021, realized_volatility=0.015, recovery_attained=1.0),
        ],
