...
def run_benchmark(n_trades=5000):
    ...
    with Session() as session:
        trades = session.query(Trade).filter(Trade.status == "CLOSED", Trade.is_deleted is False).all()
    ...
    start = time.perf_counter()
    with Session() as session:
        pnls = np.array(session.execute(select(Trade.pnl).where(Trade.status == "CLOSED", Trade.is_deleted is False)).scalars().all())
