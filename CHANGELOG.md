...
### Added
- **Volatility & Regime Safety Controls:** Hardened `RiskManager` with an extreme volatility circuit breaker and regime-adaptive confidence floors. These institutional-grade safeguards prevent execution during market anomalies and enforce higher model certainty during unstable regimes (e.g., 80% for News Shocks).
- **Adaptive Position Sizing:** Implemented confidence-scaled and volatility-aware sizing multipliers in the risk engine, ensuring conservative exposure during periods of low prediction certainty or high market turbulence.
