...
def test_calculate_position_size(risk_manager):
    ...
    size = risk_manager.size_position("XAUUSD", market_data=data, method="atr")
    assert size >= 0.01
...
def test_validate_signal_rejection(risk_manager):
    ...
    # In unified RiskManager, approve returns bool
    approved = risk_manager.approve(signal, market_data=data, open_positions_info=[])
    assert not approved
