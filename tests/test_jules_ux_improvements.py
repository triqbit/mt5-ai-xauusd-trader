...
@patch("main.open")
def test_setup_wizard_health_check_prompt(mock_open, mock_exists, mock_getpass, mock_int_prompt, mock_prompt):
    # Mock inputs for the wizard
    # 1. Mode, 2. Symbol, 3. Timeframe, 4. MT5 Account, 5. MT5 Server, 6. MetaAPI, 7. Save, 8. Health Check
...
def test_main_handles_setup_health_check(mock_get_parser, mock_run_wizard):
    ...
    args = argparse.Namespace(setup=True, check=False, doctor=False, show_config=False, log_level="INFO")
    ...
    with patch("src.core.config.get_config"), \
        ...
        mock_validate.return_value.errors = [MagicMock(field="MT5_SERVER", message="error", critical=True, remedy="remedy")]
