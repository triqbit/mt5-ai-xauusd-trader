...
def mock_capital_allocator():
    ...
    allocator.add_strategy(
        StrategyConfig(
            strategy_id="PPO_XAUUSD", symbol="XAUUSD", model_family="RL", capital_cap=50000.0
        )
    )
...
def mock_resilience_report():
    ...
    return ResilienceReport(
        ...
        scenario_results={
            "High Slippage": StressTestMetrics(
                total_return=0.08,
                max_drawdown=0.12,
                sharpe_ratio=1.1,
                win_rate=0.55,
...
def test_full_report_generation(
    ...
    report = ResearchReport(
        ...
        allocation_insights=mock_capital_allocator.to_report_section(
            rejection_history={"Heat Limit": 3}
        ),
