...
def risk_manager(trade_logger):
    ...
    cfg = TradingConfig(
        MT5_LOGIN=123456, MT5_PASSWORD="password", MT5_SERVER="server", SYMBOL="XAUUSD"
    )
...
def test_risk_manager_reconciliation_only_losses(risk_manager):
    ...
    recon_data = {"daily_pnl": -200.0, "daily_count": 2, "pnl_series": [-100.0, -100.0]}
...
def test_risk_manager_reconciliation_empty(risk_manager):
    recon_data = {"daily_pnl": 0.0, "daily_count": 0, "pnl_series": []}
