...
def test_resilience_and_circuit_breaker(mock_cfg, trade_logger, mock_monitor):
    ...
    risk.open_positions = {}
    signal_invalid = TradeSignal(symbol="INVALID", direction=1, entry_price=1.0, stop_loss=0.9, take_profit=1.1, lot_size=0.1, algorithm="test", confidence=0.9)
    approved = risk.approve(signal_invalid)
    assert approved is False

    # 5. Trigger via Low Confidence
    signal_low_conf = TradeSignal(symbol="XAUUSD", direction=1, entry_price=2300, stop_loss=2200, take_profit=2500, lot_size=0.1, algorithm="test", confidence=0.4)
...
def test_system_performance_and_resources(mock_cfg, trade_logger, sample_market_data):
    ...
    for _ in range(100):
        ...
        signal = TradeSignal(symbol="XAUUSD", direction=1, entry_price=2300, stop_loss=2200, take_profit=2500, lot_size=0.1, algorithm="test", confidence=0.8)
