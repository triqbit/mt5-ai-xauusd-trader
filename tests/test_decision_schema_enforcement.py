...
def test_risk_decision_rejection_missing_reason():
    ...
    with pytest.raises(ValidationError, match="A rejected risk decision must provide a 'reason'"):
        RiskDecision(is_approved=False, reason="", adjusted_lot_size=0.0)


def test_risk_decision_rejection_with_lot_size():
    """Rejecting while keeping a lot size > 0 should fail validation."""
    with pytest.raises(
        ValidationError, match="A rejected risk decision cannot have an adjusted lot size > 0"
    ):
        RiskDecision(is_approved=False, reason="Risk too high", adjusted_lot_size=0.01)
...
def test_execution_decision_consistency(sample_trade_signal):
    ...
    # Valid Approved
    ExecutionDecision(
        signal=sample_trade_signal, is_approved=True, confidence_score=0.8, blocked_by=None
    )
    ...
    with pytest.raises(
        ...
        ExecutionDecision(
            signal=sample_trade_signal, is_approved=True, confidence_score=0.8, blocked_by="ERROR"
        )
    ...
    with pytest.raises(
        ...
        ExecutionDecision(
            signal=sample_trade_signal, is_approved=False, confidence_score=0.8, blocked_by=None
        )
