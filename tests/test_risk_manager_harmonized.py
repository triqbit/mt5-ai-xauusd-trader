...
def test_atr_position_sizing(risk_manager, market_data):
    ...
    lots = risk_manager.calculate_position_size(
        "XAUUSD", high_vol_data, current_price=2300.0, current_atr=16.0, contract_size=100.0
    )

    # Normal volatility sizing
    normal_lots = risk_manager.calculate_position_size(
        "XAUUSD", market_data, current_price=2300.0, current_atr=5.0, contract_size=100.0
    )

    # 16/5 = 3.2 ratio -> Extreme volatility (>3.0) -> should return 0.0 lots
    assert lots == 0.0
