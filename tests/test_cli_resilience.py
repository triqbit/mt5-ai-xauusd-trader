...
def test_cli_help_resilience():
    ...
    result = subprocess.run(
        [sys.executable, "main.py", "--help"], capture_output=True, text=True, timeout=30
    )
...
def test_cli_version_resilience():
    ...
    result = subprocess.run(
        [sys.executable, "main.py", "--version"], capture_output=True, text=True, timeout=30
    )
    assert result.returncode == 0
...
def test_cli_doctor_resilience():
    ...
    result = subprocess.run(
        [sys.executable, "main.py", "--doctor"], capture_output=True, text=True, timeout=30
    )
