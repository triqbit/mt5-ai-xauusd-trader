...
def test_robustness_scoring():
    ...
    # Perfect consistency
    score = optimizer._calculate_robustness_score({"sharpe": 1.0, "mdd": 0.1}, {"sharpe": 1.0, "mdd": 0.1})
    assert score >= 0.9  # Should be high

    # OOS failure
    score = optimizer._calculate_robustness_score({"sharpe": 2.0, "mdd": 0.1}, {"sharpe": 0.2, "mdd": 0.5})
    assert score < 0.5
