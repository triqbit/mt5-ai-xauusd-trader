...
def test_macro_event_timestamps():
    ...
    event = MacroEvent(
        name="CPI", category=EventCategory.CPI, impact=EventImpact.HIGH, timestamp=ts
    )
...
def test_risk_status_block_consistency():
    ...
    status = RiskStatus(is_blocked=True, risk_multiplier=1.0, reason="Market crash")
    assert status.risk_multiplier == 0.0

    # Case 2: is_blocked=True but no reason -> should raise error
    with pytest.raises(ValidationError) as exc:
        RiskStatus(is_blocked=True, risk_multiplier=0.0, reason=None)
