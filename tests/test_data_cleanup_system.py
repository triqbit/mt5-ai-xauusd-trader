...
def test_cleanup_database_retention(test_env):
    ...
    with TradeSession() as session:
        ...
        old_unlinked = ModelSignal(
            ...
            direction=1,
            entry_price=2000.0,
            created_at=now - timedelta(days=RETENTION_UNLINKED_SIGNALS + 1),
        )

        # 2. Linked Signal to Trade (Old) -> Should be preserved
        old_linked_signal = ModelSignal(
            symbol="XAUUSD",
            direction=1,
            entry_price=2000.0,
            created_at=now - timedelta(days=RETENTION_UNLINKED_SIGNALS + 1),
        ...
        old_risk_linked_signal = ModelSignal(
            symbol="XAUUSD",
            direction=1,
            entry_price=2000.0,
            created_at=now - timedelta(days=RETENTION_UNLINKED_SIGNALS + 1),
        ...
        new_trade = Trade(
            ...
            entry_price=2000.0,
            lot_size=0.1,
            signal_id=old_linked_signal_id,
            created_at=now,
        )
        ...
        old_trade = Trade(
            ...
            entry_price=1900.0,
            lot_size=0.1,
            created_at=now - timedelta(days=RETENTION_TRADES + 1),
        )
        ...
        new_risk = RiskEvent(
            ...
            description="test",
            signal_id=old_risk_linked_signal_id,
            created_at=now,
        )
        ...
        old_risk = RiskEvent(
            event_type="CIRCUIT_BREAKER",
            description="test",
            created_at=now - timedelta(days=RETENTION_RISK_EVENTS + 1),
        )

        # Old Performance Metric -> Should be archived and purged
        old_perf = PerformanceMetric(
            sharpe_ratio=1.5, created_at=now - timedelta(days=RETENTION_PERFORMANCE_METRICS + 1)
        )
    ...
    with AuditSession() as session:
        ...
        old_audit = AuditEntry(
            action="TEST", actor="SYS", created_at=now - timedelta(days=RETENTION_AUDIT_LOG + 1)
        )
    ...
    results = cleanup_database(
        db_url, audit_db_url=audit_db_url, dry_run=False, archive_dir=archive_dir
    )
