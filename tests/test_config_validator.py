...
def base_config(monkeypatch, tmp_path):
    ...
    # Ensure institutional limits pass
...
def test_validator_mt5_password_short(monkeypatch, tmp_path):
    """Test validator fails with short MT5 password."""
    model_file = tmp_path / "model.pt"
    model_file.write_text("data")
    monkeypatch.setenv("MT5_LOGIN", "12345")
    monkeypatch.setenv("MT5_PASSWORD", "1234")
    monkeypatch.setenv("MT5_SERVER", "Broker-Demo")
    monkeypatch.setenv("MODEL_PATH", str(model_file))
    cfg = TradingConfig()
    validator = ConfigValidator(cfg)
    result = validator.validate()
    assert result.success is False
    assert any(e.field == "MT5_PASSWORD" and "too short" in e.message for e in result.errors)
...
def test_validator_risk_per_trade_critical(monkeypatch, tmp_path):
...
def test_validator_risk_per_trade_live_critical(monkeypatch, tmp_path):
    """Test risk per trade > 1% is critical in LIVE mode."""
    model_file = tmp_path / "model.pt"
    model_file.write_text("data")
    monkeypatch.setenv("MT5_LOGIN", "12345")
    monkeypatch.setenv("MT5_PASSWORD", "securepassword")
...
def test_validator_risk_warnings(monkeypatch, tmp_path):
    ...
    # Policy limit is 1%, Warning if > 1% in non-live
    monkeypatch.setenv("RISK_PER_TRADE", "0.015")
    cfg = TradingConfig()
    validator = ConfigValidator(cfg)
    result = validator.validate()
...
def test_validator_stability_guards(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("MODEL_ACCURACY_FLOOR")
...
def test_validator_sqlite_live_critical(monkeypatch, tmp_path):
    """Test SQLite in LIVE mode is critical."""
    model_file = tmp_path / "model.pt"
    model_file.write_text("data")
    monkeypatch.setenv("MT5_LOGIN", "12345")
    monkeypatch.setenv("MT5_PASSWORD", "securepassword")
    ...
    result = ConfigValidator(cfg).validate()
    assert result.success is False
    assert any(e.field == "DATABASE_URL" and e.critical for e in result.errors)
...
def test_validator_weekly_monthly_loss_limits(monkeypatch, tmp_path):
    """Test weekly loss > 10% or monthly loss > 15% is critical."""
    model_file = tmp_path / "model.pt"
    model_file.write_text("data")
    monkeypatch.setenv("MT5_LOGIN", "123456")
    monkeypatch.setenv("MT5_PASSWORD", "securepassword")
    ...
    # Weekly critical (> 10%)
    monkeypatch.setenv("MAX_WEEKLY_LOSS", "0.11")
    cfg = TradingConfig()
    result = ConfigValidator(cfg).validate()
    assert result.success is False
    ...
    monkeypatch.setenv("MAX_MONTHLY_LOSS", "0.16")
    cfg = TradingConfig()
    result = ConfigValidator(cfg).validate()
    assert result.success is False
...
def test_validator_exposure_limits(monkeypatch, tmp_path):
    ...
    # Single direction critical (> 30%) in LIVE
    ...
    # Total notional critical (> 100%) in LIVE
...
def test_validator_margin_halt_pct_critical(monkeypatch, tmp_path):
    """Test margin halt percentage > 80% is critical."""
    model_file = tmp_path / "model.pt"
    model_file.write_text("data")
    monkeypatch.setenv("MT5_LOGIN", "12345")
    monkeypatch.setenv("MT5_PASSWORD", "securepassword")
    ...
    monkeypatch.setenv("DATABASE_URL", "postgresql://user:pass@host/db")
    monkeypatch.setenv("MODEL_PATH", str(model_file))
    monkeypatch.setenv("MARGIN_HALT_PCT", "0.85")

    cfg = TradingConfig()
    result = ConfigValidator(cfg).validate()
    assert result.success is False
    assert any(e.field == "MARGIN_HALT_PCT" and e.critical for e in result.errors)
...
def test_validator_max_slippage_pips_live(monkeypatch, tmp_path):
    """Test max slippage > 1.0 is critical in LIVE mode."""
    model_file = tmp_path / "model.pt"
    model_file.write_text("data")
    monkeypatch.setenv("MT5_LOGIN", "12345")
    monkeypatch.setenv("MT5_PASSWORD", "securepassword")
...
def test_validator_max_losing_streak_live(monkeypatch, tmp_path):
    """Test max losing streak > 3 is critical in LIVE mode."""
    model_file = tmp_path / "model.pt"
    model_file.write_text("data")
    monkeypatch.setenv("MT5_LOGIN", "12345")
    monkeypatch.setenv("MT5_PASSWORD", "securepassword")
...
def test_validator_file_permissions(monkeypatch, tmp_path):
    ...
    cfg = TradingConfig()
    # Pydantic v2 SettingsConfigDict env_file is Path
    # Actually, we should check what cfg.model_config has.
    # It's a dict in Pydantic v2 for the instance but let's see.

    validator = ConfigValidator(cfg)

    # Mock os.stat to simulate insecure permissions for .env
    ...
    # Mock Path.exists to ensure the validator thinks the sensitive files exist
    from pathlib import Path as PPath
    original_exists = PPath.exists
    def mocked_exists(self):
        if self.name in (".env", "trades.db", "audit.db"):
            return True
        return original_exists(self)
    monkeypatch.setattr(PPath, "exists", mocked_exists)
