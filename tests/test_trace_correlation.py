...
def test_trace_id_propagation(audit_logger, trade_logger):
    ...
    with audit_logger.Session() as session:
        entry = session.execute(
            select(AuditEntry).where(AuditEntry.action == "test_action")
        ).scalar_one()
    ...
    with trade_logger.Session() as session:
        signal = session.execute(
            select(ModelSignal).where(ModelSignal.id == signal_id)
        ).scalar_one()
...
def test_trace_id_isolation(audit_logger):
    ...
    with audit_logger.Session() as session:
        entry1 = session.execute(
            select(AuditEntry).where(AuditEntry.action == "action1")
        ).scalar_one()
        entry2 = session.execute(
            select(AuditEntry).where(AuditEntry.action == "action2")
        ).scalar_one()
