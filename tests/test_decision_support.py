...
def test_decision_augmentation_logic(mock_explanation, mock_regime, mock_macro_risk):
    ...
    mock_macro_risk = mock_macro_risk.model_copy(update={"risk_multiplier": 0.5})  # 0.5*10 = 5
    ...
    mock_regime.confidence = 1.0
    mock_explanation.risk_assessment.risk_reward_ratio = 3.0
    mock_macro_risk = mock_macro_risk.model_copy(update={"risk_multiplier": 0.25})  # Severe reduction but not blocked
...
def test_assemble_packet_blocked_by_macro(mock_explanation, mock_regime, mock_macro_risk):
    dss = DecisionSupportSystem()
    mock_macro_risk = mock_macro_risk.model_copy(update={"is_blocked": True, "reason": "Blocked by FOMC"})
