...
mock_talib.MFI.side_effect = lambda high, low, cl, v, *a, **k: np.random.rand(len(cl))
mock_talib.CCI.side_effect = lambda high, low, cl, *a, **k: np.random.rand(len(cl))
...
def setup_mock_talib(m_talib):
    ...
    m_talib.MFI.side_effect = lambda high, low, cl, v, *a, **k: np.random.rand(len(cl))
    m_talib.CCI.side_effect = lambda high, low, cl, *a, **k: np.random.rand(len(cl))
    m_talib.MOM.side_effect = lambda c, *a, **k: np.random.rand(len(c))
    m_talib.WILLR.side_effect = lambda high, low, cl, *a, **k: np.random.rand(len(cl))
    m_talib.ULTOSC.side_effect = lambda high, low, cl, *a, **k: np.random.rand(len(cl))
