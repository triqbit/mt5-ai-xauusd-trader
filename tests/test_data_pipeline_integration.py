...
mock_talib.ATR.side_effect = lambda h, l_val, c, *a, **k: np.random.rand(len(c))
...
mock_talib.ADX.side_effect = lambda h, l_val, c, *a, **k: np.random.rand(len(c))
mock_talib.STOCH.side_effect = lambda h, l_val, c, *a, **k: (np.random.rand(len(c)), np.random.rand(len(c)))
mock_talib.OBV.side_effect = lambda c, v, *a, **k: np.random.rand(len(c))
mock_talib.MFI.side_effect = lambda h, l_val, c, v, *a, **k: np.random.rand(len(c))
mock_talib.CCI.side_effect = lambda h, l_val, c, *a, **k: np.random.rand(len(c))
...
def setup_mock_talib(m_talib):
    ...
    m_talib.ATR.side_effect = lambda h, l_val, c, *a, **k: np.random.rand(len(c))
    ...
    m_talib.ADX.side_effect = lambda h, l_val, c, *a, **k: np.random.rand(len(c))
    m_talib.STOCH.side_effect = lambda h, l_val, c, *a, **k: (np.random.rand(len(c)), np.random.rand(len(c)))
    m_talib.OBV.side_effect = lambda c, v, *a, **k: np.random.rand(len(c))
    m_talib.MFI.side_effect = lambda h, l_val, c, v, *a, **k: np.random.rand(len(c))
    m_talib.CCI.side_effect = lambda h, l_val, c, *a, **k: np.random.rand(len(c))
    m_talib.MOM.side_effect = lambda c, *a, **k: np.random.rand(len(c))
    m_talib.WILLR.side_effect = lambda h, l_val, c, *a, **k: np.random.rand(len(c))
    m_talib.ULTOSC.side_effect = lambda h, l_val, c, *a, **k: np.random.rand(len(c))
...
def test_pipeline_insufficient_history(data_generator, feature_engineer):
    ...
    with patch.object(fe_mod, "talib", mock_talib):
        ...
        mock_talib.MACD.side_effect = lambda data, *a, **k: (np.full(len(data), np.nan), np.full(len(data), np.nan), np.full(len(data), np.nan))
