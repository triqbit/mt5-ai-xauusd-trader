...
def setup_mock_talib(m_talib):
    ...
    m_talib.LINEARREG_SLOPE.side_effect = lambda d, *a, **k: np.random.rand(len(d))
    m_talib.HT_TRENDLINE.side_effect = lambda d: np.random.rand(len(d))
    m_talib.HT_DCPERIOD.side_effect = lambda d: np.random.rand(len(d))
