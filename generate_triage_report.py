...
def generate_report():
    ...
    if not GITHUB_TOKEN and max_prs_to_process > 20:
        print(
            "Warning: No GITHUB_TOKEN, limiting detailed processing to first 20 PRs to avoid rate limit."
        )
    ...
    safe_surface = [
        pr for pr in classified_prs if pr["risk"] in ["Safe Surface", "Safe Surface (Heuristic)"]
    ]
