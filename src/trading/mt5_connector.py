...
class MT5Connector:
    ...
    @with_retry(max_retries=2, exceptions=(MT5ConnectionError,))
    def initialize(self) -> bool:
        ...
        Raises:
            MT5ConnectionError: If connection to all paths fails.
        """
        ...
        if MT5_AVAILABLE:
            try:
                if mt5.initialize(
                    ...
                    logger.warning("Native mt5.initialize failed: %s", error_code)
        ...
        raise MT5ConnectionError("Could not establish connection to MT5 or MetaAPI")
    ...
    @with_retry(max_retries=3, exceptions=(MT5DataError,))
    def get_rates(self, symbol: str, timeframe: str, n_bars: int) -> pd.DataFrame:
        ...
        if not self.use_metaapi:
            ...
            if rates is None:
                err = mt5.last_error()
                logger.error("Failed to copy rates for %s: %s", symbol, err)
                raise MT5DataError(f"Could not fetch rates for {symbol}: {err}")

            df = pd.DataFrame(rates)
            if df.empty:
                raise MT5DataError(f"Fetched rates for {symbol} are empty")
    ...
    def get_tick(self, symbol: str) -> Dict[str, float]:
        ...
        if tick is None:
            err = mt5.last_error()
            logger.error("Failed to get tick for %s: %s", symbol, err)
            raise MT5DataError(f"Could not fetch tick for {symbol}: {err}")
    ...
    def place_order(self, signal: TradeSignal) -> Optional[int]:
        ...
        if not self.use_metaapi:
            ...
            if price == 0:
                raise OrderExecutionError(f"Invalid price (0.0) for {signal.symbol}")
            ...
            if result.retcode != mt5.TRADE_RETCODE_DONE:
                ...
                raise OrderExecutionError(f"Order rejected: {result.comment} (code: {result.retcode})")
