...
class RiskManager:
    ...
    def approve(
        ...
        """
        Legacy simple approval check. For full validation use validate_signal.
        """
        rejection_reason = ""
        if not self._check_circuit_breaker():
    ...
    def size_position(self, symbol: str, market_data: pd.DataFrame) -> float:
        ...
        # Approx 30 days of M5 (8640 bars) or whatever is available
    ...
    def _check_circuit_breaker(self) -> bool:
        ...
        if drawdown >= 0.15:  # Hard limit 15%
            logger.critical("CIRCUIT BREAKER: drawdown=%.1f%%", drawdown * 100)
            if self.trade_logger:
                self.trade_logger.log_risk_event(
