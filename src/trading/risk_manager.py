...
class RiskManager:
    ...
    def approve(
        ...
        if not passed:
            rejection_reason = ", ".join([k for k, v in decision_chain.items() if not v])
            logger.warning(
    ...
    def _check_exposure_limits(
        ...
        # Directional Exposure (30% net cap)
        net_lots = 0.0
        ...
        if total_notional >= (self.balance * self.cfg.max_total_notional_pct):
            return False
    ...
    def _check_symbol_allocation(self, symbol: str) -> bool:
        ...
        # For this version, we allow XAUUSD or anything in ALLOCATION_WEIGHTS
