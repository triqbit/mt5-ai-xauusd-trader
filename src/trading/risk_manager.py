...
# Ray Dalio All-Weather allocation weights (Legacy reference)
...
class RiskManager:
    ...
    def calculate_position_size(self, symbol: str, market_data: pd.DataFrame) -> float:
        ...
        # Find ATR column (resilient to different TF prefixes)
        atr_col = "atr"
        if atr_col not in market_data.columns:
            atr_cols = [c for c in market_data.columns if "atr" in c.lower()]
            if atr_cols:
                atr_col = atr_cols[0]
            else:
                logger.warning("ATR column not found in market_data, falling back to min lot size")
                return self.cfg.min_lot_size

        # Find Close column
        close_col = "close"
        if close_col not in market_data.columns:
            close_cols = [c for c in market_data.columns if "close" in c.lower()]
            if close_cols:
                close_col = close_cols[0]
            else:
                logger.warning("Close column not found in market_data")
                return self.cfg.min_lot_size

        current_atr = market_data[atr_col].iloc[-1]
        # Approx 30 days of M5 (8640 bars) - use whatever history is available
    ...
    def approve(self, signal: TradeSignal, **kwargs) -> bool:
        """Legacy approve method. Returns False for safety."""
        logger.error(
            "CRITICAL: Legacy approve() called on RiskManager. This method is deprecated "
            "and returns False to prevent unvalidated trades. Update caller to validate_signal()."
        )
        return False

    def size_position(self, symbol: str, win_rate: float = 0.5, **kwargs) -> float:
        """Legacy size_position method."""
        logger.warning("Legacy size_position() called. Use calculate_position_size() instead.")
        return self.cfg.min_lot_size
