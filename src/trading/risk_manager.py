...
Enterprise risk management engine implementing:
  ...
  - Fractional Kelly Criterion (legacy support)
...
class RiskManager:
    ...
    def validate_signal(
        ...
        Layers:
          1. Hard Circuit Breaker (Equity Drawdown).
        ...
        """
        # Layer 1: Hard Circuit Breaker (Equity Drawdown)
    ...
    def approve(
        ...
        In a real scenario, we should migrate this to validate_signal.
        """
        # Note: This legacy method lacks market_data and open_positions context.
        # It's kept for backward compatibility but marked for deprecation.
        # We'll use dummy data for context to maintain the 8-layer check.
        logger.warning("RiskManager.approve() is DEPRECATED. Use validate_signal().")
    ...
    def size_position(self, symbol: str, market_data: pd.DataFrame) -> float:
        ...
        # Sizing formula: risk_amount / (ATR * contract_size)
        # Gold contract_size is 100. For others, we'd need more logic.
        contract_size = 100 if "XAUUSD" in symbol else 1.0
        ...
        max_lots = max_notional / (price * contract_size)
    ...
    def update_equity(self, current_equity: float, realised_pnl: float = 0) -> None:
    ...
    def _check_directional_exposure(
        ...
        contract_size = 100 if "XAUUSD" in signal.symbol else 1.0
        notional = abs(net_lots) * price_estimate * contract_size
