...
Enterprise risk management engine implementing:
  ...
  - Exposure limits & net directional safeguards
  - 8-layer institutional validation cascade
...
class RiskManager:
    ...
    Enforces the institutional 8-layer validation cascade.
    """
    ...
    def validate_signal(
        ...
        if signal.symbol != self.cfg.symbol:
            # Flexible if multi-symbol support is added later, but currently restrictive
        ...
        if not self._check_risk_reward(signal):
            return RiskDecision(False, "Risk-Reward ratio below threshold")
    ...
    def calculate_position_size(self, symbol: str, market_data: pd.DataFrame) -> float:
        ...
        if market_data.empty or "atr" not in market_data.columns:
            # Fallback to config-based risk percentage if ATR is missing
            risk_amount = self.balance * self.cfg.risk_per_trade
            # Assume 100 pips risk for fallback gold sizing
            lot_size = risk_amount / 100.0
            return max(self.cfg.min_lot_size, round(lot_size, 2))

        current_atr = market_data["atr"].iloc[-1]
        # Approx 30 days of M5 (12 * 24 * 30 = 8640)
        ...
        risk_amount = self.balance * self.cfg.risk_per_trade
        # ATR * 100 converts gold ATR to $ per lot (1.0 ATR on Gold = $100 for 1 lot)
