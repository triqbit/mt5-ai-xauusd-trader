...
class TradingExecutor:
                ...
                try:
                    ...
                    with profile("data_fetch"):
                        ...
                        if last_price is not None and hasattr(self.model, "observe_outcome"):
                            ...
                            if current_price > last_price * 1.0001:
                                actual_dir = SignalDirection.BUY
                            elif current_price < last_price * 0.9999:
                    ...
                    if not risk_decision.is_approved and direction != 0:
                        self.log.warning("Risk REJECTED", reason=risk_decision.reason)
                    ...
                    if approved:
                        ...
                        if not approved:
                            self.log.warning("Filter BLOCKED", reason=filter_decision.blocked_by)
                    ...
                    if approved and direction != 0:
                        try:
                            ...
                            if ticket:
                                self.log.info("Order placed", ticket=ticket)
                            ...
                            self.log.error("Execution FAILED", error=str(e))

                    iteration_status = "success"

                except (MT5DataError, MT5ConnectionError) as e:
                    self.log.error("Market data or connection error", error=str(e))
                    time.sleep(poll_interval)
                    continue
                except Exception as e:
                    self.log.exception("Unexpected error in trading loop", error=str(e))
                    time.sleep(poll_interval)
                    continue
    ...
    def _prepare_trade_signal(
        ...
        sl = price - (direction * 2 * atr) if atr > 0 else price * 0.99
        tp = price + (direction * 4 * atr) if atr > 0 else price * 1.02
