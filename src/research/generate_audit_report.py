...
def generate_full_audit():
    ...
    regime_section = RegimeSection(
        ...
        regimes=[
            ...
            RegimeSummary(label="Ranging", frequency_pct=25.0, avg_duration_bars=12, profitability="Neutral"),
            RegimeSummary(label="News Shock", frequency_pct=10.0, avg_duration_bars=3, profitability="Low"),
        ],
    ...
    stress_section = StressTestSection(
        ...
        baseline=StressedMetric(name="Baseline", total_return="14.2%", max_drawdown="5.1%", sharpe="2.45", outcome="PASS"),
        scenarios=[
            StressedMetric(name="Spread Widening (3x)", total_return="11.8%", max_drawdown="6.4%", sharpe="2.10", outcome="PASS"),
            StressedMetric(name="Execution Hell", total_return="8.5%", max_drawdown="9.2%", sharpe="1.65", outcome="PASS"),
            StressedMetric(name="Flash Crash", total_return="-2.1%", max_drawdown="18.5%", sharpe="-0.45", outcome="FAIL"),
        ],
        fragility_indicators=["High drawdown sensitivity to price noise > 1.5 sigma", "Latency impact > 500ms degrades alpha capture"],
        failure_points=["Sudden 2% price gap in < 1 minute", "Service failure during peak London volume"]
    )
    ...
    hyper_section = HyperparameterSection(
        ...
        parameters=[
            ...
            ParameterRobustness(name="confidence_threshold", range="0.5-0.9", optimal="0.65", sensitivity="Medium"),
            ParameterRobustness(name="volatility_lookback", range="10-50", optimal="20", sensitivity="Low"),
        ],
    ...
    pattern_section = TradePatternSection(
        ...
        concentrations=[
            PatternConcentration(attribute="algo_session", value="Ensemble @ London", win_rate=0.62, profit_factor=2.45, total_trades=145),
            PatternConcentration(attribute="algo_volatility", value="Ensemble @ Normal Vol", win_rate=0.58, profit_factor=2.10, total_trades=210),
        ],
        behavioral_risks=[
            BehavioralRisk(type="Overtrading", description="High trade frequency detected in NY session (4.2 trades/hour)."),
            BehavioralRisk(type="Loss Clustering", description="Detected 2 clusters of 5+ losses during FOMC news shocks."),
        ],
    ...
    drift_section = ModelDriftSection(
        metrics=[
            DriftMetric(name="Target Distribution: close", baseline="2345.50", current="2342.10", drift_pct=2.4, status="STABLE"),
            DriftMetric(name="Return Volatility", baseline="0.0012", current="0.0018", drift_pct=25.0, status="WARNING"),
        ],
    ...
    alloc_section = AllocationSection(
        total_heat_pct=42.5,
        allocations=[
            AllocationEntry(name="ENSEMBLE_XAUUSD_M5", amount="$42,500.00", heat_pct=42.5, multiplier=1.15),
    ...
    bench_section = BenchmarkSection(
        comparisons=[
            BenchmarkComparison(name="EMA_Crossover", total_return="8.5%", sharpe="1.20", max_drawdown="12.4%", p_value="0.0012", profit_factor="1.45"),
            BenchmarkComparison(name="Momentum_ROC", total_return="6.2%", sharpe="0.95", max_drawdown="15.8%", p_value="0.0001", profit_factor="1.25"),
        ],
    ...
    print("\n" + "="*80)
    print(" GENERATING RESEARCH AUDIT REPORT ".center(80, "="))
    print("="*80 + "\n")
