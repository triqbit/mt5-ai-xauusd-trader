...
class WalkForwardOptimizer:
    ...
    def _optimize_window(
        ...
        def objective(trial: optuna.Trial) -> float:
            ...
            perturbation_penalty = self._calculate_perturbation_penalty(
                train_data, params, base_performance
            )
        ...
        return WindowResult(
            ...
            if isinstance(train_data.index[0], datetime)
            else datetime.fromtimestamp(0, tz=timezone.utc),
            train_end=train_data.index[-1]
            if isinstance(train_data.index[-1], datetime)
            else datetime.fromtimestamp(0, tz=timezone.utc),
            test_start=test_data.index[0]
            if isinstance(test_data.index[0], datetime)
            else datetime.fromtimestamp(0, tz=timezone.utc),
            test_end=test_data.index[-1]
            if isinstance(test_data.index[-1], datetime)
            else datetime.fromtimestamp(0, tz=timezone.utc),
