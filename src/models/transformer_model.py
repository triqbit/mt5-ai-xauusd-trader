...
class TimeSeriesTransformer(NN_Module):
    """
    ...
    def __init__(
        ...
        if nn is None:
            raise ImportError("torch and nn are required for TimeSeriesTransformer")
        super().__init__()
...
class PositionalEncoding(NN_Module):
    """Injects positional information into the sequence."""

    def __init__(self, d_model: int, dropout: float = 0.1, max_len: int = 5000):
        if nn is None:
            raise ImportError("torch and nn are required for PositionalEncoding")
        super().__init__()
