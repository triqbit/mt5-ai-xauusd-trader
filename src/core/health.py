...
class HealthChecker:
    ...
    def check_liveness(self) -> ComponentStatus:
        ...
        return ComponentStatus(
            status=HealthStatus.HEALTHY, message="Application is running"
        )
    ...
    def check_database(self) -> ComponentStatus:
        ...
        try:
            ...
            return ComponentStatus(
                status=HealthStatus.HEALTHY, message="Database reachable"
            )
    ...
    def check_mt5(self) -> ComponentStatus:
        ...
        if self.connector._is_initialized:
            return ComponentStatus(
                status=HealthStatus.HEALTHY, message="MT5 connection alive"
            )
        else:
            return ComponentStatus(
                status=HealthStatus.FAILED, message="MT5 connection down"
            )
    ...
    def check_models(self) -> ComponentStatus:
        ...
        loaded = []
        # Late import and checks to avoid module-level dependency on torch
        # self.model is usually a mock in tests, or an EnsembleModel in prod
        if hasattr(self.model, "_ppo_model") and getattr(self.model, "_ppo_model") is not None:
            loaded.append("PPO")
        if hasattr(self.model, "lstm_model") and getattr(self.model, "lstm_model") is not None:
            loaded.append("LSTM")
