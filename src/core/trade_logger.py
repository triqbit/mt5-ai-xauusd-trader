...
class ModelSignal(Base, AuditMixin):
    ...
    __table_args__ = (
        ...
        CheckConstraint("lot_size > 0", name="check_signal_lot_size_positive"),
        CheckConstraint(
            "confidence >= 0 AND confidence <= 1", name="check_signal_confidence_range"
        ),
        Index("ix_model_signals_symbol_created_at", "symbol", "created_at"),
    )
...
class Trade(Base, AuditMixin):
    ...
    __table_args__ = (
        ...
        Index("ix_trades_symbol_status_is_deleted", "symbol", "status", "is_deleted"),
    )
    ...
    drawdown_impact: Mapped[Optional[float]] = mapped_column(
        Float
    )  # impact on total drawdown
