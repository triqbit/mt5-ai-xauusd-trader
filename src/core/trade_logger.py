...
class AuditMixin:
    ...
    is_deleted = Column(
        Boolean, default=False, index=True, nullable=False, server_default=text("0")
    )
...
class ModelSignal(Base, AuditMixin):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "lot_size IS NULL OR lot_size > 0", name="check_signal_lot_size_positive"
        ),
    )
...
class Trade(Base, AuditMixin):
    ...
    __table_args__ = (
        ...
        CheckConstraint("direction IN (-1, 0, 1)", name="check_trade_direction_valid"),
