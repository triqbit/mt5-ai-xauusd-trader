...
class FeatureEngineer:
    ...
    def get_normalization_stats(self) -> dict[str, Any]:
        ...
        return {
            "method": self.method,
            "means": dict(zip(self.feature_columns, self.means.tolist()))
            if self.means is not None
            else None,
            "stds": dict(zip(self.feature_columns, self.stds.tolist()))
            if self.stds is not None
            else None,
            "mins": dict(zip(self.feature_columns, self.mins.tolist()))
            if self.mins is not None
            else None,
            "maxs": dict(zip(self.feature_columns, self.maxs.tolist()))
