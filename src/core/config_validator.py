...
class ConfigValidator:
    ...
    def _check_mt5_credentials(self) -> None:
        ...
        if not self.config.mt5_server or self.config.mt5_server.lower() in [
            "",
            "server_name",
            "test",
        ]:
        ...
        mt5_pass = self.config.mt5_password.get_secret_value()
        if not mt5_pass or mt5_pass.lower() in ["", "password", "test"]:
            self.errors.append(
                ValidationError(
    ...
    def _check_placeholder_secrets(self) -> None:
        ...
        if telegram_token and "YOUR_TOKEN" in telegram_token.upper():
            self.errors.append(
    ...
    def _check_risk_parameters(self) -> None:
        ...
        if self.config.risk_per_trade > 0.02:
            self.errors.append(
                ValidationError(
                    "RISK_PER_TRADE", "Risk per trade > 2% is strictly prohibited.", True
                )
            )
        ...
        if self.config.max_daily_loss > 0.15:
            self.errors.append(
                ValidationError(
                    "MAX_DAILY_LOSS", "Max daily loss > 15% is outside safe range.", True
                )
            )
