...
class ExecutionAnalyzer:
    ...
    def _evaluate_opportunity_cost(
        ...
        if signal.direction > 0:  # BUY
            ...
            would_win = False
            for h, low_val in zip(highs, lows, strict=False):
                if h >= (signal.take_profit or float("inf")):
                    would_win = True
                    break
                if low_val <= (signal.stop_loss or float("-inf")):
                    would_win = False
                    break
            ...
            # Check if TP or SL would hit first
            would_win = False
            for high_val, low_val in zip(highs, lows, strict=False):
                if low_val <= (signal.take_profit or float("-inf")):
                    would_win = True
                    break
                if high_val >= (signal.stop_loss or float("inf")):
                    would_win = False
                    break
